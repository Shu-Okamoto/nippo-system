...
## freee 連携 — 従業員単位APIが 401 invalid_access_token になる場合

事業所単位のAPI(従業員一覧)は 200 で通るのに、
`/hr/api/v1/employees/{id}/...` だけ 401 になる場合。

```json
{ "message": "ログインをして下さい", "code": "invalid_access_token" }
```
...
| code | 意味 |
|---|---|
| `invalid_authorization_company_id` | 事業所の指定が認可先と違う |
| **`invalid_access_token`** | **呼び出し元を従業員として解決できない** |

トークン自体が無効なら事業所単位のAPIも落ちるはず。片方だけ落ちるのは
トークンの問題ではなく、**エンドポイントの性質の違い**による。

### 原因

freee人事労務の `/employees/{id}/` 配下は**従業員単位のAPI**で、
呼び出し元を従業員として解決する。認可した freee アカウントが
その事業所の**従業員として登録されていない**と、誰の打刻も扱えない。

`company_admin` の権限を持っていても、それとは別に
**従業員レコードに紐づいているか**が問われる。

診断の `self_employee_id` が `null` ならこの状態。

### 対処

freee 側で、認可に使うアカウントをその事業所の従業員に紐づける。
または、既に従業員として登録されているアカウントで認可し直す。

その後もう一度診断し、`self_employee_id` に数値が入ることを確認する。

### それでも他人の打刻が通らない場合

`self_check.ok: true`(本人は通る)なのに他のメンバーが 401 のままなら、
**このAPIでは他人の打刻を代理登録できない**ということになる。

その場合は管理者トークン1つで全員分を送る構成が成立しないため、
連携方式の見直しが必要になる。`work_record_check` の結果も見て、
勤務実績(work_records)を書く方式に切り替えられるかを検討する。

---

## Step 21 — 勤怠CSVの書き出し

`25_attendance_export.sql` を移行先 SQL Editor で1回実行する
(`23_break_times.sql` 実行済みが前提)。

freee の API 連携が使えない場合の代替手段。期間内の勤怠を
全メンバー分まとめて書き出す。

### 使い方

勤怠管理 → **月別(メンバー)** タブ → 月を選ぶ →
**「📄 CSV出力(全員)」**

メンバーの選択に関係なく、その月の**全員分**が出力される。
ファイル名は `勤怠_<店舗名>_<YYYY-MM>.csv`。

### 列

```
従業員ID, 氏名, 日付, 出勤時刻, 退勤時刻, 休憩時間(分), 実働時間
```

- 従業員ID … スタッフマスタの freee従業員ID(未設定なら空)
- 出退勤・休憩 … 打刻そのまま
- 実働時間 … 15分丸め後(0.25刻み)
- 打刻がある日だけ出力する

文字コードは UTF-8(BOM付き)。Excel でそのまま開ける。

### freee に取り込む場合

freee人事労務 の勤怠インポート機能で取り込む。**freee 側のテンプレートと
列が違う場合は、Excel で並べ替えるか、こちらで列構成を合わせる。**
実際のテンプレートを確認のうえ調整すること。

---

## Step 22 — 過去分を勤務実績として送る

`26_work_record_sync.sql` を移行先 SQL Editor で1回実行する
(`25_attendance_export.sql` 実行済みが前提)。

### なぜ2種類あるか

freee には勤怠を入れる口が2つあり、用途が違う。

| API | 用途 | 使う場面 |
|---|---|---|
| 打刻 `time_clocks` | 出勤・休憩・退勤を1回ずつ順に積む | **当日分** |
| 勤務実績 `work_records` | 1日分の出退勤・休憩をまとめて設定 | **過去分** |

打刻APIは「今この瞬間に押した」ことを記録する仕組みなので、
**過去日を遡って登録することはできない**。当日分の送信が通っても
過去分が残るのはこのため。

勤務実績APIは PUT なので、同じ日に何度実行しても結果は同じ。
途中で失敗してももう一度流せばよい。

### 画面

**過去分** … 勤怠管理 → 月別(メンバー)→ 月を選択 →
**「↗ この月をfreeeへ(勤務実績)」**

メンバーの選択に関係なく、その月の全員分を送る。
**freee 側の同じ日の勤務実績は上書きされる。**

**当日分** … freee人事労務 連携 → **「freee に送信(当日分)」**

### 送信状態の整理

打刻の `freee_status` を月単位で操作できる。

| 関数 / ボタン | 動作 |
|---|---|
| `retry_freee_errors()` / 「エラーを再送対象に」 | error・skipped → pending に戻す |
| `skip_freee_punches()` / 「送信対象から外す」 | pending・error → skipped にする |

勤務実績で送った月は、打刻APIで送る必要がなくなる。
**「送信対象から外す」を実行して未送信件数を消しておく**と、
以降「未送信 N件」が残り続けずに済む。

`sent` と `manual` は両方の関数とも触らない。二重登録や、
freee 側で直した内容の上書きを避けるため。

### 出退勤が揃っていない日

勤務実績APIは出勤・退勤の両方が必要。片方しか打刻がない日は
`skipped` として送信されない。先に勤怠管理で打刻を補完すること。

---

## Step 23 — 勤務実績を丸め後の値で送る + 自動送信

`27_rounded_export.sql` を移行先 SQL Editor で1回実行する
(`26_work_record_sync.sql` 実行済みが前提)。

### 打刻と勤務実績で送る値が違う

| 送信先 | 送る値 | 理由 |
|---|---|---|
| 打刻 `time_clocks` | **実打刻そのまま** | 「実際に押した時刻」の記録なので丸めない |
| 勤務実績 `work_records` | **丸め後** | 1日の労働時間そのもの。画面の実働と一致させないと給与計算が合わない |

`get_attendance_export` が丸め後の値も返すようにした。
...
`work_minutes = rounded_end - rounded_start - rounded_break` なので、
freee 側の実働が月別ビューの表示と完全に一致する。

`clock_summary()` は画面で実打刻を出す必要があるため変更していない。
丸めが要るのは書き出しだけなので、そこに閉じ込めている。

### 休憩の送り方

実際の休憩が47分でも、丸めて60分として送る必要がある。実打刻の
入り/戻りをそのまま送ると freee 側の計算が画面とずれるため、
**丸めた合計と一致する休憩1本**にまとめて送る。

開始時刻は実際の休憩入りに寄せ、勤務時間内に収まるよう調整する。

```
実打刻   08:46 出勤 / 12:03 休憩入 / 12:50 休憩戻 / 17:14 退勤
送信内容 09:00 出勤 / 12:03〜13:03 休憩 / 17:00 退勤 → 実働 7h
```
...
`vercel.json` に Cron を追加済み。**Vercel 側の設定は不要**で、
デプロイすると有効になる。

```json
{ "crons": [{ "path": "/api/cron/freee-daily", "schedule": "0 9 * * *" }] }
```

`0 9 * * *` は **UTC 9:00 = 日本時間 18:00**。閉店後に当日分の
勤務実績を全店舗ぶん送る。

### 認証

`CRON_SECRET` を環境変数に設定しておくと、Vercel Cron が
`Authorization: Bearer <CRON_SECRET>` を自動で付けてくる。
このヘッダが一致しない限り実行されない。

手動でテストする場合:

```
curl -H "x-cron-secret: <CRON_SECRET>" https://<host>/api/cron/freee-daily
```

### 二重送信にならない

勤務実績は PUT なので、同じ日に何度実行しても結果は同じ。
自動送信と手動の「この月をfreeeへ」を併用しても問題ない。

### 送信されない日

- 従業員IDが未設定
- 出勤か退勤のどちらかが欠けている
- 丸めた結果 退勤 <= 出勤 になる(1分だけの打刻など)

これらは `skipped` として数えられ、送信されない。

### 時刻を変えたい

`vercel.json` の `schedule` を書き換える(UTC で指定)。
...
**Vercel の Hobby プランは Cron が1日1回まで。** それ以上の頻度が
必要な場合は Pro プランが要る。
...
```
(1) 当日の打刻   time_clocks   実打刻をそのまま送る
(2) 当日の勤務実績 work_records 丸め後の労働時間で確定させる
```

(2) は PUT なので (1) から計算された値を上書きする。結果として
**freee には実際に押した時刻の記録が残りつつ、労働時間は丸め後**になる。

(1) が失敗しても (2) は実行される。給与計算に必要な値は入る。

### 打刻の送信は当日分のみ

freee の打刻APIは過去日を受け付けない。過去日を送ると 400 で弾かれる。

```
{"type":"bad_request","messages":["打刻の時間が正しくありません。"]}
```

さらに出勤が入らないと、続く休憩・退勤が

```
{"type":"bad_request","messages":["打刻の種類が正しくありません。"]}
```

となって連鎖的に失敗する。

そのため自動送信・手動送信とも**打刻は当日分だけ**を対象にする。
以前は日付を問わず未送信を古い順に送っていたため、溜まった過去分から
処理が始まって大量に失敗していた。

**画面の日付選択は表示の絞り込みで、送信対象とは無関係。**

過去分は勤務実績(work_records)で送る。こちらは過去日でも通る。

---

## Step 25 — 休憩の丸めルール修正

`28_break_rounding_rule.sql` を移行先 SQL Editor で1回実行する。

### ルール

休憩は**次の15分区切りまで上げる**。区切りちょうどの値も次に上がる。
...
`CEIL(45/15)*15 = 45` となり、45分がそのまま45分になってしまう。
区切りちょうどでも次に上げる必要があるため、計算式が違う。

```
誤: CEIL(min / 15) * 15
正: (FLOOR(min / 15) + 1) * 15     ※ 0分は 0分のまま
```
...
出勤は切り上げ、退勤は切り捨てのまま。**区切りちょうどの時刻は
そのまま採用する**(09:00 は 09:00、09:01 は 09:15)。
休憩とは扱いが違う点に注意。

### 実装箇所

丸めが3箇所にあるため、すべて同じルールに揃えている。

| 対象 | 実装 |
|---|---|
| 打刻の実働計算 | `clock_summary()` → `round_break_minutes()` |
| freee送信・CSV | `get_attendance_export()` → `round_break_minutes()` |
| 日報の実績入力 | `src/lib/calc.ts` の `roundBreakMinutes()` |

SQL 側は `nippo.round_break_minutes()` に切り出して1か所にまとめた。

---

## Step 26 — 給与明細の案内をLINEで配る

`29_freee_mypage.sql` を移行先 SQL Editor で1回実行する。

LINE で `[給料]` と送られたときに、freee 従業員マイページのURLと
その人のログインIDを案内するための情報を持たせる。

### パスワードは保持しない・配らない

**LINE でパスワードを配ってはいけない。** トーク履歴に残り、転送・
スクリーンショット・端末紛失で給与情報が漏れる。

freee は招待メールから**本人がパスワードを設定する**方式なので、
そもそも管理者が知る必要がない。配るのは **URLとログインID まで**。

忘れた場合はログイン画面の「パスワードを忘れた方」から本人が再設定する。

### ログインIDの置き場所

`staff` ではなく `staff_private` に置く。`staff` はアプリが
`select('*')` で読んでおり anon から見えるため。

### 画面

管理画面 → スタッフマスタ

1. 上部の **「freee マイページURL」** に URL を設定(全員共通)
2. 各メンバーの **「freeeログインID」** 列にメールアドレスを入力
3. 同じ列の **「文面」** ボタンで LINE 配布用テキストをコピー
...
※パスワードを忘れた場合はログイン画面の
  「パスワードを忘れた方」から再設定してください。
```

これを DX 側の `[給料]` 応答に登録する。個人打刻URLと同じく、
**メンバーごとに違う文面**になる。

### アプリではなく Web URL を配る

freee にはスマホアプリもあるが、LINE から配るなら Web の URL がよい。
未インストールでもそのまま開けるため。アプリを入れている端末では
URL から自動でアプリが開くことも多く、URL を配れば両方カバーできる。

---

## Step 27 — 勤怠の月別(店舗)ビュー

`30_attendance_month_matrix.sql` を移行先 SQL Editor で1回実行する
(`28_break_rounding_rule.sql` 実行済みが前提)。

勤怠管理に3つ目のビューを追加する。

| ビュー | 縦 | 横 | 用途 |
|---|---|---|---|
| 日別(店舗) | メンバー | 打刻明細 | その日の打刻を直す |
| **月別(店舗)** | **日付** | **メンバー** | 月全体を俯瞰する |
| 月別(メンバー) | 日付 | 打刻明細 | 1人の1か月を確認する |

### 並びの向き

日付を行、メンバーを列にしている。1か月=30行は縦スクロールで自然に
読め、メンバー10人程度なら横に収まるため。日付列は横スクロールしても
左端に固定される。

### 月間レポートとの違い

見た目は月間レポート下部の表と同じだが、**データ元が違う**。

| | データ元 |
|---|---|
| 月間レポート | 日報の実績入力(`shift_entries`) |
| **勤怠管理の月別(店舗)** | **打刻(`time_clock_events`)** |

同じ形で並べることで、実績入力と打刻のズレを見比べられる。

人時売も打刻ベースで計算する(売上 ÷ その日の打刻合計時間)。
実績入力側の人時売と混ぜると何を見ているのか分からなくなるため。

### 表示

- セルは実働時間(15分丸め後、0.25刻み)
- 最下部に メンバーごとの合計 と 人時売平均
- 上部に 出勤メンバー数 / 合計実働 / 店舗の人時売
- 土日は背景色を変える

CSV出力と「この月をfreeeへ(勤務実績)」もこのビューから実行できる。

---

## Step 28 — 月別(店舗)のタイムカード表示

`31_matrix_timecard.sql` を移行先 SQL Editor で1回実行する
(`30_attendance_month_matrix.sql` 実行済みが前提)。
...
```
日付     │ 榎木            │ 藤川          │ 日計
 1(火)  │ 09:01〜17:14    │ 12:58〜17:03  │ 11h
        │ 休 12:03〜12:50 │ 休なし        │
        │ 7h  休47分      │ 4h            │
```

セルは3行。出退勤 / 休憩の入り〜戻り / 実働と休憩合計。

休憩が1日に複数回あればその分だけ2行目が増える。
戻り打刻がない場合は赤字で **中**(休憩中)と表示する。

### 表示の切替

上部の **タイムカード / 時間数のみ** で切り替えられる。
メンバーが増えて横に収まらなくなったら「時間数のみ」にする。

### 値の扱い

- 時刻は**打刻そのまま**(丸めない)
- 実働は**15分丸め後**
- 退勤の打刻がない日は赤字で **未** と表示する

日別ビューや月別(メンバー)と同じ扱い。

### 合計行

メンバーごとの合計実働に加えて**出勤日数**も出す。
月次の確認で「何日出たか」がすぐ分かるようにするため。

---

## Step 29 — ログインIDをメールアドレス以外に対応

`32_freee_login_id.sql` を移行先 SQL Editor で1回実行する
(`29_freee_mypage.sql` 実行済みが前提)。
...
の形式で、メールアドレスではなかった。29 ではメール形式で検証して
いたため入力できない。

### 変更点

- 列名を `freee_login_email` → **`freee_login_id`** に変更
  (29 実行済みなら rename、未実行なら新規追加。どちらでも通る)
- 関数を `set_staff_login_email` → **`set_staff_login_id`** に変更
- 検証をメール形式から「空白なし・200文字以内」に緩和。
  メールアドレスでも `satonoajimikawa-00000010` でも通る

配布用の文面は変わらない。「ログインID: ◯◯」として出力される。
