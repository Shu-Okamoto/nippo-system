...
-- 勤怠の月別(店舗)ビュー
-- ------------------------------------------------------------
-- 1か月の実働時間を メンバー × 日付 の表で返す。
-- 画面では日付を行、メンバーを列に並べる(1か月=30行は縦スクロールで
-- 自然に読め、メンバー10人程度なら横に収まるため)。
-- データは行列どちらにも並べ替えられる形で返す。
--
-- 月間レポートは日報の実績入力(shift_entries)を見ているが、
-- こちらは勤怠管理なので打刻(time_clock_events)を見る。
-- 同じ形で並べることで、実績入力と打刻のズレを見比べられる。
--
-- 人時売も打刻ベースで計算する。売上 ÷ その日の打刻合計時間。
-- 実績入力側の人時売と混ぜると意味が分からなくなるため。
--
...
BEGIN
  ...
  RETURN (
    ...
    -- 対象は打刻があるメンバーだけ。在籍していても打刻が無ければ出さない
    m AS (
    ...
    member_rows AS (
      SELECT
        ...
        c.name,
        c.sort_order,
        jsonb_agg(COALESCE(c.work_minutes, 0) ORDER BY c.work_date) AS cells,
        COALESCE(SUM(c.work_minutes), 0)::int AS total_minutes,
        -- 出勤日の人時売を、本人のその日の実働時間で加重平均する
        CASE
          WHEN COALESCE(SUM(
